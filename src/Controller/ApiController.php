...
class ApiController extends AbstractController
{
    ...
    #[Route('/dist/{package}/{version}-r{revision}-{reference}.{type}',
        ...
        requirements: [
            ...
            'revision' => Requirement::POSITIVE_INT,
            'reference' => '[a-z0-9]+',
            'type' => '(zip)',
        ],
        methods: ['GET'],
    ...
    public function packageDistribution(Request $request, int $revision, string $reference, string $type): Response
    {
        ...
        if ($reference !== $metadata->getReference()) {
            throw $this->createAccessDeniedException();
        }

        if (!$this->distributionResolver->resolve($metadata, $type, $this->asyncApiRequests)) {
            throw $this->createNotFoundException();
        }
    ...
    /**
     * Fetch mirrored package distributions through the API.
     *
     * The endpoint to fetch mirrored distributions is separate because when Composer requests a mirror, it doesn't
     * know which revision to fetch, so we default to the latest revision with the specified reference.
     */
