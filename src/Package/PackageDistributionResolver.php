...
readonly class PackageDistributionResolver
{
    ...
    public function resolve(Version $version, ?string $reference, ?string $type, bool $async): bool
    {
        ...
        return match (true) {
            $this->buildDistributions && !$hasDistribution => $this->build($version, $reference ?? $version->getSourceReference(), $type ?? $version->getSourceType()),
            $this->mirrorDistributions && $hasDistribution => $this->mirror($version, $reference ?? $version->getDistReference(), $type ?? $version->getDistType()),
            default => false,
        };
    }
